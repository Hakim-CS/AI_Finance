...
app.get('/loans', protect, async (req, res) => {
  try {
    const result = await pool.query(
...
app.post('/loans', protect, async (req, res) => {
...
// PUT update a loan
app.put('/loans/:id', protect, async (req, res) => {
  const { id } = req.params;
  const { type, amount, person, date, description } = req.body;
  try {
    const result = await pool.query(
...
app.patch('/loans/:id/toggle', protect, async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query(
      `UPDATE "Loan"
       SET status = CASE WHEN status = 'open' THEN 'settled' ELSE 'open' END,
           settled_at = CASE WHEN status = 'open' THEN CURRENT_TIMESTAMP ELSE NULL END,
           updated_at = CURRENT_TIMESTAMP
...
// DELETE a loan
app.delete('/loans/:id', protect, async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query(
